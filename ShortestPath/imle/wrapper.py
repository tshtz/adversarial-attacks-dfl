        ...
        class WrappedFunc(torch.autograd.Function):
            ...
            def forward(ctx, input: Tensor, *args):
                ...
                if noise_distribution is None:
                    noise = torch.zeros(size=perturbed_input_shape).to(input.device)
                else:
                    noise = noise_distribution.sample(shape=torch.Size(perturbed_input_shape)).to(
                        input.device
                    )
                ...
                # [BATCH_SIZE, N_SAMPLES, ...]
                perturbed_input_3d = (
                    input.view(batch_size, 1, -1)
                    .repeat(1, nb_samples, 1)
                    .view(perturbed_input_shape)
                )
            ...
            def backward(ctx, dy):
                ...
                ####  New line added
                dy = (
                    dy.view(batch_size, 1, -1)
                    .repeat(1, nb_samples, 1)
                    .view([batch_size * nb_samples] + list(instance_shape))
                )
