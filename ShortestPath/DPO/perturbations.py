...
def sample_noise_with_gradients(noise, shape):
    ...
    if noise not in SUPPORTED_NOISES:
        raise ValueError(
            "{} noise is not supported. Use one of [{}]".format(noise, SUPPORTED_NOISES)
        )
...
def perturbed(func=None, num_samples=1000, sigma=0.05, noise=_NORMAL, batched=False, device=None):
