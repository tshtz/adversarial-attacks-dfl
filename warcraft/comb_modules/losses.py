...
class NCE_c(torch.nn.Module):
    ...
    def forward(self, pred_weights, true_weights, target, cache):
        ...
        for ii in range(len(pred_weights)):
            loss += (
                (mm * (target[ii] - cache) * (pred_weights[ii] - true_weights[ii])).sum(dim=(1))
            ).mean()
        loss /= len(pred_weights)
        return loss
...
class PairwiseLoss(torch.nn.Module):
    def __init__(self, tau=0.0, minimize=True, mode="B"):
        super().__init__()
        self.tau = tau
        self.mm = 1 if minimize else -1
        self.mode = mode
    ...
    def forward(self, pred_weights, true_weights, target, cache):
        ...
        for ii in range(len(pred_weights)):
            _, indices = np.unique(
                (self.mm * true_weights[ii] * cache).sum(dim=(1, 2)).detach().cpu().numpy(),
                return_index=True,
            )
            ...
            loss += relu(
                self.tau
                + (cache[big_ind] * pred_weights[ii] - cache[small_ind] * pred_weights[ii]).sum(
                    dim=(1, 2)
                )
            ).mean()
        loss /= len(pred_weights)
        ...
        for ii in range(len(pred_weights)):
            ...
            loss += (
                (
                    ...
                    - (cache[big_ind] * true_weights[ii] - cache[small_ind] * true_weights[ii]).sum(
                        dim=(1, 2)
                    )
                )
                .square()
...
class ListwiseLoss(torch.nn.Module):
    def __init__(self, tau=1.0, minimize=True):
        super().__init__()
        self.tau = tau
        self.mm = 1 if minimize else -1
    ...
    def forward(self, pred_weights, true_weights, target, cache):
        ...
        for ii in range(len(pred_weights)):
            loss += -(
                F.log_softmax((-mm * pred_weights[ii] * cache / tau).sum(dim=(1, 2)), dim=0)
                * F.softmax((-mm * true_weights[ii] * cache / tau).sum(dim=(1, 2)), dim=0)
            ).mean()
        loss /= len(pred_weights)
