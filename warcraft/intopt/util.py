...
def convert_to_np(A_trch=None, b_trch=None, G_trch=None, h_trch=None):
    ...
    if (A_trch is None) and (G_trch is None):
        raise Exception(
            "The problem is (trivially) unbounded because there are no non-trivial constraints."
        )
    elif A_trch is None:
        m_ub, n = G_trch.shape
        G_np = G_trch.detach().cpu().numpy().astype(float)
        h_np = h_trch.detach().cpu().numpy().astype(float)
        ...
        m_eq, n = A_trch.shape
        A_np = A_trch.detach().cpu().numpy()
        b_np = b_trch.detach().cpu().numpy()
        ...
        if n_ub != n_eq:
            raise Exception(
                "Number of variables in the equality constraint matrix does not match."
