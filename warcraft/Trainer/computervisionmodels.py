...
def get_model(model_name, out_features, in_channels):
    ...
    own_models = {
        ...
        "CombResnet18": CombRenset18,
    }

    if model_name in preloaded_models:
        model = preloaded_models[model_name](pretrained=False, num_classes=out_features)
        ...
        return own_models[model_name](out_features=out_features, in_channels=in_channels)
    else:
...
class ConvNet(torch.nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=channels_1, kernel_size=kernel_size, stride=stride
        )
        self.conv2 = nn.Conv2d(
            in_channels=channels_1, out_channels=channels_2, kernel_size=kernel_size, stride=stride
        )
...
class PureConvNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=channels_1, kernel_size=kernel_size, stride=1
        )
        self.conv2 = nn.Conv2d(
            in_channels=channels_1, out_channels=channels_2, kernel_size=kernel_size, stride=1
        )
        ...
        self.conv3 = nn.Conv2d(
            in_channels=channels_2 if use_second_conv else channels_1,
            out_channels=1,
            kernel_size=1,
            stride=1,
        )
