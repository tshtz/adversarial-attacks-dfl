...
from .utils import maybe_parallelize, shortest_pathsolution
...
class IntoptDifflayer(nn.Module):
    def __init__(self, shape, thr=1e-8, damping=1e-8, diffKKT=False) -> None:
        ...
        self.C, self.d = torch.from_numpy(C), torch.from_numpy(d)
        self.N, self.V = N, V
        self.intoptsolver = intopt(
            self.A, self.b, None, None, thr=thr, damping=damping, dopresolve=True, diffKKT=diffKKT
        )
