...
from .util import convert_to_np, standardizeLP
...
def intopt_nonbacthed(
    ...
    class WrappedFunc_cls(Function):
        ...
        def forward(ctx, c_trch):
            ...
            x_solve = (
                torch.from_numpy(standardizer.transformsolution(x / tau)).float().to(c_trch.device)
            )
        ...
        def backward(ctx, dx):
            ...
            if diffKKT:
                ...
                M = A_.dot(
                    Dinv.reshape(-1, 1) * A_.T
                )  #### we don't divide by mu because it would cancel out
                ...
                deltau = (x + w2.T.dot(c_) - t2.T.dot(b_)) / (
                    b_.dot(t1) - c_.dot(w1) + (kappa / tau)
                )
                ####  delx = w2 + w1.T @ deltau
                ...
                delx_torch = (
                    torch.from_numpy(standardizer.transformsgradient(delx)).float().to(dx.device)
                )
...
class intopt(nn.Module):
    ...
    def __init__(
        ...
        self.net = intopt_nonbacthed(
            A_trch, b_trch, G_trch, h_trch, thr, damping, diffKKT, dopresolve
        )
