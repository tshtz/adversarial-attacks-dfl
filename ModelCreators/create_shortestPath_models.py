...
from ShortestPath.Trainer import utils
from ShortestPath.Trainer.data_utils import ShortestPathDataModule
from ShortestPath.Trainer.PO_modelsSP import *  # noqa: F403


def get_modelclass_from_globals(
    model_type: Literal["ShortestPath"],
    modelname: Literal[
        "baseline", "SPO", "DBB", "CachingPO", "IMLE", "FenchelYoung", "IntOpt", "DCOL"
    ],
):
...
def dataset_from_config(params: dict) -> Tuple[ShortestPathDataModule, dict]:
    input_params = {
        "N": params["N"],
        "noise": params["noise"],
        "deg": params["deg"],
    ...
    dataset = ShortestPathDataModule(**input_params)
    return dataset, input_params


def shortestPath_train_wrapper(
    name: str,
    dataset: ShortestPathDataModule,
    ...
    :type dataset: ShortestPathDataModule
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if log_to_mlflow:
            ...
            logger.experiment.log_param(
                key="model_architecture_short", value=architecture_desc, run_id=logger.run_id
            )
        # Monitor metric
        ...
        if params["model"] == "CachingPO":
            init_cache = utils.batch_solve(
                dataset.solver, torch.from_numpy(dataset.y_train), relaxation=False
            )
            model = modelcls(architecture=architecture, init_cache=init_cache, **params)
        else:
            model = modelcls(architecture=architecture, **params)
