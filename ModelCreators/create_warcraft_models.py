...
from warcraft.Trainer.data_utils import WarcraftDataModule
...
def dataset_from_config(params: dict) -> Tuple[WarcraftDataModule, dict]:
    # Get the inputs fo for the WarcraftDataModule
    input_params = {
    ...
    dataset = WarcraftDataModule(**input_params)
    return dataset, input_params


def warcraft_train_wrapper(
    name: Literal["Warcraft"],
    dataset: WarcraftDataModule,
    ...
    :type name: Literal["Warcraft"]
    :param dataset: The dataset to use
    :type dataset: WarcraftDataModule
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        architecture_type = "ResNet18" if params["model"] == "baseline" else "CombResnet18"
        ...
        # Monitor metric
        # Check if model contains baseline or not
        ...
        if params["model"] == "CachingPO":
            cache = data.return_trainlabel()
            model = modelcls(
                data_output_shape=dataset.metadata["output_shape"],
